            ...
            <div className='container mx-auto p-5'>
                <div className='flex justify-center space-x-4 mb-6'>
                    {categories.map((category) => (
                        <button key={category} onClick={() => setActiveCategory(category)} className={`px-4 py-2 rounded-md font-medium ${activeCategory === category ? 'bg-purple-600 text-white' : "bg-gray-200 text-gray-700"}`}>
                            {category}
                        </button>
                    ))}
                </div>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6'>
                    {filteredImages.map((image) => (
                        <div key={image.id} className='bg-white rounded-md shadow-lg'>
                            <img src={image.img} alt={image.title} className='w-full h-52 object-cover rounded-t-md' />
                            <div className='p-4'>
